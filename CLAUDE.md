...
- Time check-in output contract and cleaner: `src/hermes-checkin.js` prompts Hermes to wrap the check-in message in `output::` and `::end` delimiters (1-3 short sentences). `src/hermes-checkin-cleaner.js` strips `<think>`/`</think>` thinking tags (including those spanning across the prompt-echoed instructions), then extracts content between the first `output::`/`::end` pair, strips remaining session metadata lines, model normalization/provider logs, message-count footers, and prompt-echo lines; rejects content without terminal punctuation (catches scaffolding bleed-through); falls back to `buildFallbackMessage` when invalid. This delimiter-based contract keeps the cleaner minimal and robust against new bleed-through shapes.
