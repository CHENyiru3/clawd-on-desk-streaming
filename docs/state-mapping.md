...
| macOS keyboard activity | composing | Light pre-submit cue | Falls back to thinking if the theme has no `composing` asset | Falls back to thinking if the theme has no `composing` asset |
